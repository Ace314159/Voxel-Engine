...
public:
	...
	int getChunkCoord(int worldCoord) const {
		...
		return worldCoord / CHUNK_LEN;
	}
	int getWorldCoord(int chunkCoord, unsigned int blockCoord) const {
	...
	float getLightLevel(unsigned int x, unsigned int y, unsigned int z);
	void setLightLevel(unsigned int x, unsigned int y, unsigned int z, float lightLevel);
};
